...
const props = defineProps<{ title: string; rows: BreakdownRow[]; empty?: string; selected?: string | null }>()
  ...
  <section class="card p-5">
    ...
    <ul class="space-y-1.5">
      <li
        v-for="row in rows"
        :key="row.value"
        class="relative flex items-center gap-3 rounded-md px-2.5 py-1.5 overflow-hidden cursor-pointer select-none hover:bg-[color-mix(in_srgb,var(--ink)_4%,transparent)]"
        :title="selected === row.value ? 'Clear filter' : `Filter dashboard by ${row.value}`"
      ...
      >
        <span
          ...
          :style="{ width: pct(row) + '%' }"
        />
        <span class="relative flex-1 truncate text-sm" :class="{ 'font-medium text-[var(--accent)]': selected === row.value }">{{ row.value }}</span>
