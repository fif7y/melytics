...
class McpController extends Controller
{
    ...
    private const TOOLS = [
        ...
        'get_event_props' => ['/sites/{site_id}/event-props', ['event', 'prop', 'by']],
    ...
    private function toolList(): array
    {
        ...
        return [
            ...
            ['name' => 'get_event_props', 'description' => 'Break down a custom event by one of its properties. A string property returns a value distribution; a numeric property returns an aggregate (sum/avg/count); a numeric property with "by" set is summed per group (e.g. purchase value by product = revenue by product)', 'inputSchema' => $schema($range + [
                'event' => ['type' => 'string', 'description' => 'Custom event name (from get_breakdown with dimension=event)'],
                'prop' => ['type' => 'string', 'description' => 'Property key on that event'],
                'by' => ['type' => 'string', 'description' => 'Optional categorical property to group a numeric prop by'],
            ], ['site_id', 'event', 'prop'])],
